...
from download import validate_url, get_request, get_soup_image, choice
...
def test_get_url_data_HTTPError():
    with pytest.raises(SystemExit):
        assert get_request('http://httpbin.org/status/403')


def test_get_url_data_timeout():
    with pytest.raises(SystemExit):
        assert get_request('http://httpbin.org/status/408')


def test_get_url_data_soup():
    url = 'https://www.google.com'
    test = get_request(url)
    assert get_request(url) == test
